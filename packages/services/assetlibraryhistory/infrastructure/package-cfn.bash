...
DESCRIPTION
    Packages the asset library history service, ready for deployment.
...
if [ -z "$AUTH" ]; then
  ...
  template_file='infrastructure/cfn-assetLibraryHistory-noAuth.yml'
else
  template_file='infrastructure/cfn-assetLibraryHistory.yml' 
fi
...
echo '
**********************************************************
  Packaging the Asset Library History CloudFormation template and uploading to S3
**********************************************************
'
aws cloudformation package \
  --template-file $template_file \
  --output-template-file build/cfn-assetLibraryHistory-output.yml \
