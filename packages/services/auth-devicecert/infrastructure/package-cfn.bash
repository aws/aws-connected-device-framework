...
DESCRIPTION
    Packages the device cert custom-auth library, ready for deployment.
...
logTitle 'Packaging the DeviceCert Auth CloudFormation template and uploading to S3'
aws cloudformation package \
  --template-file infrastructure/cfn-auth-devicecert.yaml \
  --output-template-file build/cfn-auth-devicecert-output.yaml \
  --s3-bucket $DEPLOY_ARTIFACTS_STORE_BUCKET \
  $AWS_ARGS

logTitle 'DeviceCert Auth packaging done!'
