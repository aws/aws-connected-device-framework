...
describe('CreateDeviceCertificateStepProcessor', () => {
    ...
    beforeEach(() => {
        ...
        const certificateExpiryDays=128;

        instance = new CreateDeviceCertificateStepProcessor(mockedIotFactory, mockedSSMFactory, certificateExpiryDays);
    });

    // aws-sdk-mock mocks cannot be made within an async method, therefore must mock the call OUTSIDE the test method
    const mockedCACertificate = {
    ...
    AWSMock.mock('Iot', 'describeCACertificate', (_params:AWS.Iot.Types.DescribeCACertificateRequest, callback?: (err: AWS.AWSError, data: AWS.Iot.Types.DescribeCACertificateResponse) => void) => {
        callback(null, mockedCACertificate);
    });
    mockedIot = new AWS.Iot();

    // aws-sdk-mock mocks cannot be made within an async method, therefore must mock the call OUTSIDE the test method
    const mockedSsmParameter = {
        Parameter: {
    ...
    AWSMock.mock('SSM', 'getParameter', (_params:AWS.SSM.Types.GetParameterRequest, callback?: (err: AWS.AWSError, data: AWS.SSM.Types.GetParameterResult) => void) => {
        callback(null, mockedSsmParameter);
    });
    ...
    it('device certificate processor should create device cert and key', async () => {
        ...
        expect(output.parameters.CaCertificatePem).toEqual(mockedCACertificate.certificateDescription.certificatePem);
