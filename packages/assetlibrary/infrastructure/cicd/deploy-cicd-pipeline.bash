...
OPTIONAL ARGUMENTS
	-r (string)   Name of CodeCommit repo (defaults to cdf-assetlibrary).
	-g (string)   Name of service git branch (defaults to master).
	-h (string)   Name of cdf-infrastructure-* git branch (defaults to master).
	-l (string)   Name of cdf-libraries git branch (defaults to master).
...
while getopts ":b:e:r:g:h:i:l:v:s:n:t:c:a:R:P:" opt; do
  case $opt in
    ...
    l  ) export LIBRARIES_BRANCH=$OPTARG;;
...
if [ -z "$LIBRARIES_BRANCH" ]; then
  export LIBRARIES_BRANCH=master
	echo -l LIBRARIES_BRANCH not provided, therefore defaulted to $LIBRARIES_BRANCH
fi
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
      ...
      LibrariesBranch=$LIBRARIES_BRANCH \
