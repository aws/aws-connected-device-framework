...
export class AwsIotThing<S, T, U> {
    ...
    constructor(
        ...
        this.mqttClient.on('timeout', (_thingName, clientToken) => {
            logger.verbose(`clientToken:${clientToken}`, {
                ...this.CLASS_LOGGING_DATA,
                method: 'timeout',
                type: 'in',
            });
        });
    ...
    private onDeltaHandler(thingName: string, delta: DeviceDelta<U>): void {
        ...
        logger.silly(`thingName:${thingName}, delta:${JSON.stringify(delta)}`, {
            ...logMeta,
            type: 'in',
        });
    ...
    public updateShadow(clientToken: string, reported: T, desired?: S): void {
        ...
        logger.silly(`clientToken:${clientToken}, reported:${JSON.stringify(reported)}`, {
            ...logMeta,
            type: 'in',
        });
    ...
    public publish(topic: string, message: unknown): Promise<void> {
        ...
        logger.silly(`topic:${topic}, message:${JSON.stringify(message)}`, {
            ...logMeta,
            type: 'in',
        });
        ...
        this.mqttClient.subscribe(topic, { qos: 0 }, (err: unknown) => {
            ...
            if (err) {
                ...
                this.mqttClient.on('message', (messageTopic: string, message: string) => {
                    ...
                    logger.silly(`messageTopic: ${messageTopic}, message: ${message}`, {
                        ...logMeta3,
                        type: 'in',
                    });
