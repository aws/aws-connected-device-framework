...
export class VpcEndpointCustomResource implements CustomResource {
    ...
    public async create(customResourceEvent: CustomResourceEvent) : Promise<unknown> {
        ...
        logger.debug(`VpcEndpointCustomResource: create: in: customResourceEvent: ${JSON.stringify(customResourceEvent)}`);
        ...
        const routeTableIds = (customResourceEvent.ResourceProperties.RouteTableIds || []) as VpcEndpointRouteTableIdList;
        let policyDocument = customResourceEvent.ResourceProperties.PolicyDocument || '';
        const privateDnsEnabled = (customResourceEvent.ResourceProperties.PrivateDnsEnabled==='true');
        const subnetIds = (customResourceEvent.ResourceProperties.SubnetIds || []) as VpcEndpointSubnetIdList;
        const vpcEndpointType = customResourceEvent.ResourceProperties.VpcEndpointType || 'Gateway';
        ...
        if (existingEndpoints.length === 0) {
            ...
            const result:AWS.EC2.CreateVpcEndpointResult = await this.createVpcEndpoint(vpcConfig);

            logger.debug(`VpcEndpointCustomResource: createVpcEndpoint: out: result: ${JSON.stringify(result)}`);
    ...
    public async delete(customResourceEvent: CustomResourceEvent): Promise<unknown> {
        logger.debug(`VpcEndpointCustomResource: delete: in: customResourceEvent: ${JSON.stringify(customResourceEvent)}`);
    ...
    private async getVpcEndpoint(vpcId: string, endpoint: string): Promise<AWS.EC2.Types.VpcEndpoint[]> {

        logger.debug(`VpcEndpointCustomResource: getVpcEndpoint: in: vpcId: ${JSON.stringify(vpcId)}, endpoint: ${endpoint}`);
        ...
        logger.debug(`VpcEndpointCustomResource: getVpcEndpoint: out: VpcEndpoints: ${JSON.stringify(result)}`);

        return result.VpcEndpoints;
    }

    private async createVpcEndpoint(vpcConfig: AWS.EC2.CreateVpcEndpointRequest): Promise<AWS.EC2.CreateVpcEndpointResult> {
        logger.debug(`VpcEndpointCustomResource: createVpcEndpoint: in: vpcConfig: ${JSON.stringify(vpcConfig)}`);
