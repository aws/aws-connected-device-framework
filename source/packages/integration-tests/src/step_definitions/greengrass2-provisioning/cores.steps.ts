...
const coresService: CoresService = container.get(
    GREENGRASS2_PROVISIONING_CLIENT_TYPES.CoresService
);
...
Given(
    'greengrass2-provisioning core device {string} does not exist',
...
Given(
    'greengrass2-provisioning core device {string} exists with attributes:',
...
Then(
    'config file for {string} exists with attributes:',
    async function (name: string, data: DataTable) {
        let config: unknown;
        try {
...
When(
    'I create greengrass2-provisioning core task with attributes:',
    async function (data: DataTable) {
        delete world.lastCoreTaskId;
        try {
            const task: NewCoreTask = buildModel(data);
            world.lastCoreTaskId = await coresService.createCoreTask(
                task,
...
When(
    'I install new greengrass2-provisioning core device {string} on EC2',
    async function (coreName: string) {
        try {
            ...
            try {
                fs.copyFileSync(
                    `${__dirname}/../../../../src/testResources/ggv2_core_device_install_script.txt`,
                    bootstrapScript
                );
...
When('I delete greengrass2 core {string}', async function (coreName: string) {
    try {
        await coresService.createCoreTask(
            {
                type: 'Delete',
                coreVersion: '2.4.0',
                cores: [{ name: coreName, provisioningTemplate: 'EMPTY' }],
            },
            getAdditionalHeaders(world.authToken)
        );
    } catch (err) {
...
When(
    'I wait until greengrass2-provisioning core device {string} is {string}',
    { timeout: 120 * 1000 },
    async function (coreName: string, deviceStatus: string) {
        let core;
        try {
        ...
        while (core?.device?.status !== deviceStatus) {
            console.log(
                `core ${coreName} is ${core?.device?.status} but waiting to be ${deviceStatus}\n`
            );
...
Then(
    'last greengrass2-provisioning core task exists with attributes:',
    async function (data: DataTable) {
        let task: CoreTask;
        try {
            task = await coresService.getCoreTask(
                world.lastCoreTaskId,
                getAdditionalHeaders(world.authToken)
            );
        } catch (err) {
            world.errStatus = err.status;
