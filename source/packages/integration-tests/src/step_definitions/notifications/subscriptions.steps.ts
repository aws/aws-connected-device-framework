...
const messagesService: MessagesDebugService = container.get(
    NOTIFICATIONS_CLIENT_TYPES.MessageDebugService
);
...
Given(
    ...
    async function (principalValue: string, userId: string) {
        ...
        const subId = await getSubscriptionIdFromPrincipal(
            eventsourcesService,
            eventsService,
            subscriptionsService,
            this,
...
Given(
    ...
    async function (principalValue: string, userId: string) {
        ...
        this[USER_ID] = userId;
        this[SUBSCRIPTION_ID] = await getSubscriptionIdFromPrincipal(
            eventsourcesService,
            eventsService,
            subscriptionsService,
            this,
...
When('I create a subscription with attributes', async function (data: DataTable) {
    ...
    try {
        ...
        this[SUBSCRIPTION_ID] = await createSubscription(
            subscriptionsService,
            this,
            eventId,
            data
...
Then('last subscription exists with attributes', async function (data: DataTable) {
    ...
    try {
        r = await subscriptionsService.getSubscription(
            id,
...
Then('no subscriptions exist for event {string}', async function (eventName: string) {
    ...
    try {
        await subscriptionsService.listSubscriptionsForEvent(
            eventId,
            undefined,
            getAdditionalHeaders(this[AUTHORIZATION_TOKEN])
        );
