...
When('I create account with attributes', async function (data: DataTable) {
    ...
    try {
        ...
        world.lastAccountId = await accountsService.createAccount(
            account,
...
Given(
    'account {string} associated with organizational unit {string}',
    async function (accountId: string, organizationalUnitId: string) {
        const accounts = await accountsService.listAccounts(organizationalUnitId);
        expect(
            accounts.accounts.find((o) => o.accountId === accountId),
            `${accountId} not associated with ${organizationalUnitId}`
...
Then('last account exists with attributes', async function (data: DataTable) {
    ...
    try {
        account = await accountsService.getAccount(
            world.lastAccountId,
            getAdditionalHeaders(world)
        );
    } catch (err) {
