...
Given('organizational unit {string} exists', async function (organizationalUnitId: string) {
    const organizationalUnit = await organizationalUnitService.getOrganizationalUnit(organizationalUnitId, getAdditionalHeaders(world))
    expect(organizationalUnit.id).equal(organizationalUnitId)

});

Given('organizational unit {string} does not exists', async function (organizationalUnitId: string) {
    try {
        await organizationalUnitService.getOrganizationalUnit(organizationalUnitId, getAdditionalHeaders(world))
...
Then('last organizationalUnit exists with attributes', async function (data: DataTable) {
    ...
    try {
        organizationalUnit = await organizationalUnitService.getOrganizationalUnit(world.lastOrganizationalUnitId, getAdditionalHeaders(world))
    } catch (err) {
...
When('I create organizationalUnit with attributes', async function (data: DataTable) {
    ...
    try {
        ...
        world.lastOrganizationalUnitId = await organizationalUnitService.createOrganizationalUnit(organizationalUnitResource, getAdditionalHeaders(world));
    } catch (err) {
        world.errStatus = err.status;
