...
import {
    ACMPCA_TEMPLATE_NAME,
    AWS_ISSUED_CERTIFICATE_TEMPLATE_NAME,
...
When('I provision a thing {string} using a csr', async function (thingName: string) {
    ...
    const provisionThingResponse: ProvisionThingResponse = await thingService.provisionThing(
        provisionThingRequest,
...
Then(
    'thing {string} belongs to thing group {string}',
    async function (thingName: string, thingGroupName: string) {
        thingName = replaceTokens(thingName);
        thingGroupName = replaceTokens(thingGroupName);
        const r = await iot.listThingGroupsForThing({ thingName }).promise();
        const exists =
            (r?.thingGroups?.filter((tg) => tg.groupName === thingGroupName)?.length ?? 0) > 0;
        expect(exists).eq(true);
    }
);

Then(
    'thing {string} does not belong to thing group {string}',
    async function (thingName: string, thingGroupName: string) {
        thingName = replaceTokens(thingName);
        thingGroupName = replaceTokens(thingGroupName);
        const r = await iot.listThingGroupsForThing({ thingName }).promise();
