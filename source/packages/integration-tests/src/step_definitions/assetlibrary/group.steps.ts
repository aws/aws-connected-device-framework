...
When(
    'I create group {string} of {string} with attributes',
    async function (name: string, parentPath: string, data: DataTable) {
        try {
...
When(
    'I create group {string} of {string} applying profile {string} with attributes',
    async function (name: string, parentPath: string, profileId: string, data: DataTable) {
        try {
...
When(
    'I create group {string} of {string} with invalid attributes',
    async function (name: string, parentPath: string, data: DataTable) {
        try {
            await createGroup(this, name, parentPath, data);
            fail('Expected 400');
...
When(
    'I update group {string} with attributes',
    async function (groupPath: string, data: DataTable) {
        const d = data.rowsHash();
...
When(
    'I update group {string} applying profile {string}',
    async function (groupPath: string, profileId: string) {
        const group: Group20Resource = {
            groupPath,
            templateId: undefined,
        };
...
When(
    ...
    async function (thisGroup: string, otherGroup: string, relation: string) {
        try {
            await groupService.detachFromGroup(
                thisGroup,
                relation,
                otherGroup,
                getAdditionalHeaders(this)
...
When(
    'I attach group {string} to group {string} via {string}',
    async function (thisGroup: string, otherGroup: string, relation: string) {
        try {
            await groupService.attachToGroup(
                thisGroup,
                relation,
                otherGroup,
                getAdditionalHeaders(this)
...
When(
    'I retrieve {string} group members of {string}',
    async function (template: string, groupPath: string) {
        if (template === '') {
            template = undefined;
        }
        try {
            this['members'] = await groupService.listGroupMembersGroups(
                groupPath,
                template,
                undefined,
                undefined,
...
When(
    'I retrieve {string} device members of {string}',
    async function (template: string, groupPath: string) {
        if (template === '') {
            template = undefined;
        }
        try {
            this['members'] = await groupService.listGroupMembersDevices(
                groupPath,
                template,
                undefined,
                undefined,
...
When(
    'I retrieve groups related of {string} with {string} relationship and following parameters:',
    async function (groupPath: string, relationship: string, data: DataTable) {
        ...
        try {
            this['members'] = await groupService.listGroupRelatedGroups(
                groupPath,
                relationship,
                template,
                direction,
