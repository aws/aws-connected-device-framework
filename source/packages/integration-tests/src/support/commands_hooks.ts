...
import { TemplatesService, TemplateModel } from '@aws-solutions/cdf-commands-client';
import { container } from '../di/inversify.config';
import { COMMANDS_CLIENT_TYPES } from '@aws-solutions/cdf-commands-client/dist';
...
const COMMANDTEMPLATES_TEMPLATE_FEATURE_TEMPLATE_IDS: string[] = [
    'testCommandTemplate',
    'testCommandTemplate2',
];
const COMMANDTEMPLATES_COMMAND_FEATURE_TEMPLATE_IDS: string[] = [
    'testCommandsFeatureTemplateSimple',
    'testCommandsFeatureTemplateParams',
    'testCommandsFeatureTemplateFiles',
];
...
async function deleteCommandTemplates(world: unknown, ids: string[]) {
    for (const id of ids) {
        await templates.deleteTemplate(id, getAdditionalHeaders(world)).catch((_err) => {
