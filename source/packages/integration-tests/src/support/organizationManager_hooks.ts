...
async function tearDown() {
    function suppressException(_reason: unknown) {
        // Do nothing with the error.
    }

    for (const account of testAccounts) {
        await accountsService.deleteAccount(account).catch(suppressException);
    }
    for (const ou of testOrganizationalUnits) {
        await organizationalUnitService.deleteOrganizationalUnit(ou.id).catch(suppressException);
    }
