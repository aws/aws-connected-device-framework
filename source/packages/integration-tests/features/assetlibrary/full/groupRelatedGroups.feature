Feature: Group lifecycle
  ...
  Scenario: Create a new group Template
    ...
    When I create and publish the root authorized assetlibrary group template "TEST-groups-groupTemplate002" with attributes
      ...
      | required   | ["gSysUserId"]                                                                                                                             |
      | relations  | {"out": {"owner": [{"name": "TEST-groups-groupTemplate001", "includeInAuth": true}], "parent": [{"name": "root", "includeInAuth": true}]}} |
  ...
  #5
  Scenario: Can't retrieve related group with error offset/count parameter
    Given my authorization is
      | / | * |
    When I retrieve groups related of "/TEST-groups-group001" with "owner" relationship and following parameters:
      | templateId | test-groups-grouptemplate002 |
