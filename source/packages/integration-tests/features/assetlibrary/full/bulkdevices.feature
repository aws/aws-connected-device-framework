Feature: Bulk Device Operations
  ...
  Scenario: Setup
    Given published assetlibrary device template "TEST-bulkdevices-type" exists
  ...
  Scenario: Bulk Get Devices Mix of Existing and Non-Existing Devices
    Given device "TEST-bulkdevices-device001" exists
  ...
  Scenario: Bulk Get Non-Existing Devices
    Given device "TEST-bulkdevices-device888" does not exist
    And device "TEST-bulkdevices-device999" does not exist
  ...
  Scenario: Teardown
    Given draft assetlibrary device template "TEST-bulkdevices-type" does not exist
