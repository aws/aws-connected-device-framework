Feature: Device Patch Deployment
  ...
  Scenario: Setup
    Given device-patch-deployment template "integration_test_template" exists
  ...
  Scenario: Create a deployment task for an edge device which isnt activated as a managed instance
    ...
    When I create a patch deployment Task for "IntegrationTestCore1" edge device with attributes
      | deploymentTemplateName | integration_test_template                                                               |
  ...
  Scenario: Teardown
    Given device-patch-deployment template "integration_test_template" does not exist
