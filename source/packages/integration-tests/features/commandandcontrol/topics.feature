Feature: Command & Control - Topics
  ...
  Scenario: Send a message to multiple things
    ...
    When I send command-and-control message to last command with attributes:
      | targets |{"awsIoT": {"thingNames": ["cdf-integration-test-cac-device1","cdf-integration-test-cac-device2"]}} |
    ...
    Then last command-and-control message exists with attributes:
      ...
      | $.targets.awsIoT.thingNames.[0] | cdf-integration-test-cac-device1 |
      | $.targets.awsIoT.thingNames.[1] | cdf-integration-test-cac-device2 |
  ...
  Scenario: A recipient replies to a message
    When thing "cdf-integration-test-cac-device1" replies to last command-and-control message as "accepted"
    And thing "cdf-integration-test-cac-device1" replies to last command-and-control message with payload:
      | response | first |
    And thing "cdf-integration-test-cac-device1" replies to last command-and-control message with payload:
      | response | second |
    When I pause for 3000ms
