...
const getStackResourceSummaries = async (
    ...
    const getStackResourceId = (key: string): string => {
        return inMemoryStackResourceSummaries[stackName]?.find((r) => r.LogicalResourceId === key)
            ?.PhysicalResourceId;
    };
    return getStackResourceId;
};
...
const getStackParameters = async (
    ...
    const getStackResourceId = (key: string): string => {
        return inMemoryStackParameters[stackName].find((p) => p.ParameterKey === key)
            ?.ParameterValue;
    };
    return getStackResourceId;
};
...
type packageStackParams = Pick<
    packageAndDeployStackParams,
    'answers' | 'templateFile' | 'cwd' | 'needsPackaging' | 'serviceName'
> & { parameterOverrides?: string[] };
...
const packageAndUploadTemplate = async ({
    ...
    const artifactPrefix =
        optionalDeploymentPrefix !== undefined ? optionalDeploymentPrefix : 'cloudformation';
    ...
    }) as { Parameters: { [key: string]: unknown } };
    ...
    const parametersBasedOnAnswers = parameterOverrides.map((str) => {
        ...
        if (value !== 'undefined') return { ParameterKey: key, ParameterValue: value };
        else return { ParameterKey: key, ParameterValue: '' };
    });

    const allParameters: CloudFormationParameterList =
            ...
            : Object.keys(templateParameters)
                  ...
                  .map((key) => {
                      return {
                          ...
                          ParameterKey: key,
                          ParameterValue: templateParameters[key]['Default']
                              ? templateParameters[key]['Default']
                              : '',
                      };
                  });
...
const packageAndDeployStack = async ({
    ...
    const deployArgs = [
        ...
        answers.region,
        '--tags',
        `cdf_service=${serviceName}`,
        `cdf_environment=${answers.environment}`,
        ...tagsList.asCLIOptions(),
    ];
