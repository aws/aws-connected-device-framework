    ...
    private generateApplicationConfiguration(answers: Answers): string {
        ...
        if (answers.provisioning.setPcaAliases) {
            if (!answers.provisioning.pcaAliases.list.includes(answers.provisioning.pcaAlias)) {
                answers.provisioning.pcaAliases.cas?.push({
                    alias: answers.provisioning.pcaAlias,
                    value: answers.provisioning.pcaArn,
                });
            }
        }
        ...
        try {
            ...
            if (aliases.list.length == 0) {
                const config = await lambda.getFunctionConfiguration({
                    FunctionName: `cdf-provisioning-rest-${answers.environment}`,
                });
        ...
        const questions = [
            {
                ...
                when(answers: Answers) {
                    return (
                        answers.provisioning?.setPcaAliases === true && pcaAliases.list?.length > 1
                    );
                },
            },
            {
                ...
                when(answers: Answers) {
                    return (
                        answers.provisioning?.setPcaAliases === true &&
            ...
            },
            {
                ...
                when(answers: Answers) {
                    return (
                        answers.provisioning?.setPcaAliases === true &&
                        (answers.provisioning.pcaAliases.list?.length === 1 ||
                            answers.provisioning.pcaAlias === 'Create New PCA Alias')
                    );
                },
            },
            {
        ...
        const questions = [
            {
                ...
                when(answers: Answers) {
                    return (
                        answers.provisioning?.setIotCaAliases === true &&
            ...
            },
            {
                ...
                when(answers: Answers) {
                    return (
                        answers.provisioning?.setIotCaAliases === true &&
                        (answers.provisioning.iotCaAliases.list?.length === 1 ||
                            answers.provisioning.iotCaAlias === 'Create New AWS IoT CA alias')
                    );
                },
            },
            {
