...
export class PatchDao {
    ...
    public async save(patch: PatchItem): Promise<void> {
        ...
        const params = {
            ...
            Item: {
                ...
                si1Sort: createDelimitedAttribute(PkType.DevicePatch, patch.patchStatus, patch.patchId),
                si2Hash: createDelimitedAttribute(PkType.PatchTemplate, patch.patchTemplateName, PkType.PatchTemplateVersion, patch.patchTemplate.versionNo),
            ...
            const patchRecord: AWS.DynamoDB.DocumentClient.WriteRequest = {
                PutRequest: {
                    Item: {
                        ...
                        sk: createDelimitedAttribute(PkType.Device, patch.deviceId),
                        si1Sort: createDelimitedAttribute(PkType.DevicePatch, patch.patchStatus, patch.patchId),
                        si2Hash: createDelimitedAttribute(PkType.PatchTemplate, patch.patchTemplateName, PkType.PatchTemplateVersion, patch.patchTemplate?.versionNo || null),
                        createdAt: patch.createdAt?.toISOString(),
                        updatedAt: patch.updatedAt?.toISOString(),
                        patchTemplateName: patch.patchTemplateName,
                        patchStatus: patch.patchStatus,
    ...
    public async getBulk(patchItems: { patchId: string, deviceId: string }[]): Promise<PatchItem[]> {
    ...
    public async list(deviceId: string, status?: string, count?: number, exclusiveStart?: PatchListPaginationKey): Promise<[PatchItem[], PatchListPaginationKey]> {
    ...
    public async update(patch: PatchItem): Promise<void> {
        ...
        if (patch.updatedAt && patch.updatedAt instanceof Date) {
            logger.silly(`patch.dao: update: using updated at from payload, updatedAt: ${patch.updatedAt}`);
        ...
        const params = {
            ...
            UpdateExpression: 'set patchStatus = :s, statusMessage = :m, updatedAt = :u, patchTemplateName = :t, si1Sort = :si1Sort',
            ExpressionAttributeValues: {
                ':s': patch.patchStatus,
    ...
    public async delete(patchId: string): Promise<void> {
        ...
        const queryParams: AWS.DynamoDB.DocumentClient.QueryInput = {
            ...
            ExpressionAttributeValues: { ':hash': createDelimitedAttribute(PkType.DevicePatch, patchId) }
        };
