...
export class MessagesController implements interfaces.Controller {
    ...
    public async createMessage(@requestParam('commandId') commandId: string, @requestBody() resource: NewMessageResource, @response() res: Response) : Promise<void> {
        logger.info(`messages.controller createMessage: in: commandId:${commandId}, resource: ${JSON.stringify(resource)}`);
        try {
            ...
            res.status(201)
                ...
                .setHeader('x-messageid', messageId);
    ...
    public async getMessage(@requestParam('messageId') messageId: string, @response() res: Response) : Promise<MessageResource> {
    ...
    public async listMessages(@requestParam('commandId') commandId: string, 
        ...
        logger.debug(`messages.controller listMessages: in: commandId:${commandId}, fromCreatedAtExclusive:${fromCreatedAtExclusive}, count:${count}`);

        try {
            const [items, paginationKey] = await this.service.listMessages(commandId, {createdAt:fromCreatedAtExclusive}, count);
    ...
    public async listReplies(@requestParam('messageId') messageId: string, 
        ...
        logger.debug(`messages.controller listReplies: in: messageId:${messageId}, thingName:${thingName}, fromReceivedAtExclusive:${fromReceivedAtExclusive}, count:${count}`);

        try {
            const [items, paginationKey] = await this.service.listReplies(messageId, thingName,{receivedAt:fromReceivedAtExclusive}, count);
    ...
    public async delete(@requestParam('messageId') messageId: string, @response() res: Response) : Promise<void> {
