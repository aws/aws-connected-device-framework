...
describe('Workflow.Job', () => {
  ...
  beforeEach(() => {
    ...
    mockedThingsService.getBulkProvisionTask = jest.fn().mockResolvedValue({ taskId: '222', status: 'Completed' })

    underTest = new JobAction('1234', 5, 'us-west-2', 'fakeArnRole', mockedThingsService, mockedIotFactory);

  });

  const message: MessageItem = {
    commandId: '1111',
    resolvedTargets: [{ id: 'thing1', type: 'thing', status: 'pending' }, { id: 'thing2', type: 'thing', status: 'pending' }]
  }
  ...
  it('should resolve thing names as targetArns', async () => {
    ...
    const createJobRequest = (mockedIot.createJob as jest.Mock).mock.calls[0][0] as Iot.CreateJobRequest
  ...
  it('should create ephmeral groupArn when things exceed maximum target limit', async () => {
    underTest = new JobAction('1234', 1, 'us-west-2', 'fakeArnRole', mockedThingsService, mockedIotFactory);
    await underTest.process(message, command)
    expect(mockedIot.createJob).toBeCalledTimes(1)
    const createJobRequest = (mockedIot.createJob as jest.Mock).mock.calls[0][0] as Iot.CreateJobRequest
  ...
  it('should handle both things and groups in resolved target list', async () => {
    ...
    underTest = new JobAction('1234', 2, 'us-west-2', 'fakeArnRole', mockedThingsService, mockedIotFactory);
    await underTest.process(message, command)
    expect(mockedIot.createJob).toBeCalledTimes(1)
    const createJobRequest = (mockedIot.createJob as jest.Mock).mock.calls[0][0] as Iot.CreateJobRequest
    expect(createJobRequest.targets.length).toBe(2)
