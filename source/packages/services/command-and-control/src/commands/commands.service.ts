...
import { MessageListPaginationKey, MessageItem } from '../messages/messages.models';
...
export class CommandsService {
    ...
    public async list(tags?:Tags, exclusiveStart?: CommandListPaginationKey, count?:number): Promise<[CommandItem[],CommandListPaginationKey]> {
        logger.debug(`commands.service list: in: tags:${JSON.stringify(tags)}, exclusiveStart:${JSON.stringify(exclusiveStart)}, count:${count}`);
    ...
    public async listIds(tags:Tags, exclusiveStart?: CommandListPaginationKey, count=this.MAX_LIST_RESULTS): Promise<[string[],CommandListPaginationKey]> {
        logger.debug(`commands.service listIds: in: tags:${JSON.stringify(tags)}, exclusiveStart:${JSON.stringify(exclusiveStart)}, count:${count}`);
        ...
        const resultsForTagsFutures:Promise<[string[],CommandListIdsByTagPaginationKey]>[] = new Array(tagCount);
        for (let tagIndex = 0; tagIndex < tagCount; tagIndex++) {
            ...
            resultsForTagsFutures[tagIndex] = this.commandDao.listIds(tagKeys[tagIndex], tagValues[tagIndex], paginationKey, count);
        }
        ...
        const getNextPageOfResults = async (tagIndex:number) : Promise<boolean> => {
            ...
            resultsForTags[tagIndex] = await this.commandDao.listIds(tagKeys[tagIndex], tagValues[tagIndex], paginationKey, count);
    ...
    public async update(updated: CommandItem) : Promise<void> {
        ...
        if (updated.deliveryMethod?.type!==undefined && existing.deliveryMethod.type!==updated.deliveryMethod.type) {
        ...
        const merged = merge(existingAsJson, updatedAsJson, { arrayMerge: (_, updated) => updated}) as CommandItem;
    ...
    private async sqsSendCommandForDeletion(commandId:string) : Promise<SendMessageResult> {
        ...
        logger.debug(`commands.service sqsSendCommandForDeletion: params: ${JSON.stringify(params)}`);
