...
export class SubscriptionDao {
    ...
    public constructor(
        ...
        @inject('aws.dynamoDb.tables.eventConfig.gsi2KeySk')
        private eventConfigGsi2KeyGsi2Sk: string,
        @inject('aws.dynamoDb.tables.eventConfig.gsi2KeyGsi2Sort')
        private eventConfigGsi2KeyGsi2Sort: string,
    ...
    public async create(si: SubscriptionItem): Promise<void> {
        ...
        const gsi2Key = createDelimitedAttribute(
            PkType.EventSource,
            si.eventSource.id,
        ...
        if (si.targets) {
            for (const targetType of Object.keys(si.targets)) {
                ...
                for (const target of targets) {
                    const putItem = this.targetDao.buildPutItemAttributeMap(
                        target,
                        si.eventSource.id,
                        si.eventSource.principal,
                        si.principalValue,
    ...
    public async update(si: SubscriptionItem): Promise<void> {
        ...
        if (si.ruleParameterValues) {
            params.UpdateExpression = 'set ruleParameterValues=:rp';
            params.ExpressionAttributeValues[':rp'] = si.ruleParameterValues;
        }

        if (si.enabled !== undefined) {
            params.UpdateExpression = !params.UpdateExpression
                ? 'set enabled=:re'
                : `${params.UpdateExpression}, enabled=:re`;
            params.ExpressionAttributeValues[':re'] = si.enabled;
        }
    ...
    public async listSubscriptionsForEventMessage(
        ...
        const params: DocumentClient.QueryInput = {
            ...
            ExpressionAttributeValues: {
                ':value': createDelimitedAttribute(
                    PkType.EventSource,
                    eventSourceId,
                    principal,
                    principalValue,
            ...
            ProjectionExpression:
                'address,attributeMapping,conditions,disableAlertThreshold,enabled,eventSourceId,#name,phoneNumber,pk,platformApplicationArn,platformEndpointArn,principal,principalValue,ruleParameterValues,sk,snsTopicArn,subscriptionArn,subscriptionId,tableName,targetType,#token,topic',
        };
    ...
    public async listSubscriptionIdsForEventUserPrincipal(
        eventSourceId: string,
        eventId: string,
        principal: string,
        principalValue: string,
