...
container
    .bind<string>('aws.dynamoDb.tables.eventNotifications.name')
    .toConstantValue(process.env.AWS_DYNAMODB_TABLES_EVENTNOTIFICATIONS_NAME);
container
    .bind<string>('aws.dynamoDb.tables.eventConfig.gsi1')
    .toConstantValue(process.env.AWS_DYNAMODB_TABLES_EVENTCONFIG_GSI1);
container
    .bind<string>('aws.dynamoDb.tables.eventConfig.gsi2KeySk')
    .toConstantValue(process.env.AWS_DYNAMODB_TABLES_EVENTCONFIG_GSI2KEYSK);
container
    .bind<string>('aws.dynamoDb.tables.eventConfig.gsi2KeyGsi2Sort')
    .toConstantValue(process.env.AWS_DYNAMODB_TABLES_EVENTCONFIG_GSI2KEYGSI2SORT);
...
container
    .bind<string>('aws.sqs.asyncProcessing')
    .toConstantValue(process.env.EVENTSPROCESSOR_AWS_SQS_ASYNCPROCESSING);
container
    .bind<string>('aws.lambda.dynamoDbStream.name')
    .toConstantValue(process.env.AWS_LAMBDA_DYNAMODBSTREAM_NAME);
container
    .bind<string>('aws.lambda.lambdaInvoke.arn')
    .toConstantValue(process.env.AWS_LAMBDA_LAMBDAINVOKE_ARN);

container
    .bind<EventSourceService>(TYPES.EventSourceService)
    .to(EventSourceService)
    .inSingletonScope();
container.bind<EventSourceDao>(TYPES.EventSourceDao).to(EventSourceDao).inSingletonScope();
container
    .bind<EventSourceAssembler>(TYPES.EventSourceAssembler)
    .to(EventSourceAssembler)
    .inSingletonScope();

container
    .bind<DynamoDbEventSource>(TYPES.DynamoDbEventSource)
    .to(DynamoDbEventSource)
    .inSingletonScope();
container
    .bind<IotCoreEventSource>(TYPES.IotCoreEventSource)
    .to(IotCoreEventSource)
    .inSingletonScope();
container
    .bind<ApiGatewayEventSource>(TYPES.ApiGatewayEventSource)
    .to(ApiGatewayEventSource)
    .inSingletonScope();
container
    .bind<ApigwTriggerService>(TYPES.ApigwTriggerService)
    .to(ApigwTriggerService)
    .inSingletonScope();
...
container
    .bind<EventConditionsUtils>(TYPES.EventConditionsUtils)
    .to(EventConditionsUtils)
    .inSingletonScope();
...
container
    .bind<SubscriptionAssembler>(TYPES.SubscriptionAssembler)
    .to(SubscriptionAssembler)
    .inSingletonScope();
...
container
    .bind<DDBStreamTransformer>(TYPES.DDBStreamTransformer)
    .to(DDBStreamTransformer)
    .inSingletonScope();
