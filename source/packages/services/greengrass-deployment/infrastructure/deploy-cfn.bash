...
while getopts ":e:c:b:v:g:n:i:a:y:z:C:A:k:R:P:" opt; do
  case $opt in
...
logTitle 'Greengrass Deployment Identifying deployed endpoints'

stack_exports=$(aws cloudformation list-exports $AWS_ARGS)


greengrass_provisioning_table_export="$GREENGRASS_PROVISIONING_STACK_NAME-table"
greengrass_provisioning_table=$(echo $stack_exports \
    | jq -r --arg greengrass_provisioning_table_export "$greengrass_provisioning_table_export" \
    '.Exports[] | select(.Name==$greengrass_provisioning_table_export) | .Value')

cat $CONFIG_LOCATION | \
  jq --arg greengrass_provisioning_table "$greengrass_provisioning_table" \
  ' .aws.dynamoDB.ggProvisioningTable=$greengrass_provisioning_table' \
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
      ...
      GreengrassProvisioningTable=$greengrass_provisioning_table \
