...
export class TemplatesController implements interfaces.Controller {
    ...
    public async listTemplates(
        ...
        try {
            const [items, paginationKey] = await this.templatesService.list(count, {
    ...
    public async listTemplateVersions(
        @requestParam('name') name: string,
        @queryParam('count') count: number,
        ...
        try {
            const [items, paginationKey] = await this.templatesService.listVersions(name, count, {
                version: exclusiveStartVersion,
            });
