...
export class DevicesDao {
    ...
    private assemble(items: DocumentDbClientItem[]): DeviceItem[] {
        ...
        items.forEach(item => {
            ...
            } else if (sk.length === 4 && sk[0] === PkType.ClientDevice && sk[2] === PkType.Artifact) {
    ...
    public async list(failedOnly: boolean, count: number, exclusiveStart: DeviceListPaginationKey): Promise<[DeviceItem[], DeviceListPaginationKey]> {
        logger.debug(`devices.dao list: in: failedOnly:${failedOnly}, count:${count}, exclusiveStart:${JSON.stringify(exclusiveStart)}`);
    ...
    private generateListQuery(count?: number, exclusiveStart?: DeviceListPaginationKey): QueryCommandInput {
    ...
    private generateListFilteredByFailedDeployments(count?: number, exclusiveStart?: DeviceListPaginationKey): QueryCommandInput {

        const thingNameDbId = createDelimitedAttribute(PkType.ClientDevice, exclusiveStart.thingName);
