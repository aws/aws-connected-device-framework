...
export class CoreTasksService {
    ...
    public async disassociateDevicesFromCoreAsync(task: CoreTaskItem, core: CoreItem, deprovisionClientDevices: boolean): Promise<void> {
        logger.debug(`coreTasks.service disassociateDevicesFromCoreAsync: in: task:${JSON.stringify(task)}, core:${JSON.stringify(core)}`);
        ...
        try {
            const listDevicesResponse = await this.ggv2.send(new ListClientDevicesAssociatedWithCoreDeviceCommand({ coreDeviceThingName: core.name }))
            if (listDevicesResponse.associatedClientDevices.length > 0) {
                const devicesToDelete = listDevicesResponse.associatedClientDevices.map(o => { return { coreName: core.name, name: o.thingName, provisioningTemplate: placeHolderProvisioningTemplate } })
                const deviceTaskId = await this.deviceTaskService.create(
                    { coreName: core.name, devices: devicesToDelete, type: 'Delete', options: { deprovisionClientDevices: deprovisionClientDevices } }, core.name)
                await this.publishCoreTaskStatusCheck(task.id, deviceTaskId)
            } else {
                const deletedCore = await this.coresService.deleteCore(task, core.name, deprovisionClientDevices)
    ...
    private async publishCoreTaskStatusCheck(coreTaskId: string, deviceTaskId: string, counter = 0): Promise<void> {
        logger.debug(`coreTasks.service publishCoreTaskStatusCheck: in: coreTaskId:${coreTaskId}, deviceTaskId:${deviceTaskId}`);
    ...
    public async updateCoreTaskStatus(coreTaskId: string, deviceTaskId: string, counter: number): Promise<void> {
        logger.debug(`coreTasks.service updateCoreTaskStatus: in: coreTaskId:${coreTaskId}, deviceTaskId:${deviceTaskId}, counter: ${counter}`);

        const { taskStatus, statusMessage, coreName } = await this.deviceTaskService.get(deviceTaskId)
        ...
        try {
            if (taskStatus === 'Failure') {
                ...
                await this.coresService.deleteCore(coreTask, core.name, coreTask.options?.deprovisionCores)
            }
    ...
    public async processDeleteCoreTaskBatch(taskId: string, cores: CoreItem[]): Promise<void> {
        logger.debug(`coreTasks.service processDeleteCoreTaskBatch: in: taskId:${taskId}, cores:${JSON.stringify(cores)}`);
        try {
            // validation
            ow(taskId, ow.string.nonEmpty);
            ...
            await this.disassociateDevicesFromCoreAsync(task, core, task.options?.deprovisionClientDevices)
        } catch (e) {
            logger.error(`coreTasks.service processDeleteCoreTaskBatch: e: ${e.name}: ${e.message}`);
        }
    ...
    public async processCreateCoreTaskBatch(taskId: string, cores: CoreItem[]): Promise<void> {
        logger.debug(`coreTasks.service processCreateCoreTaskBatch: in: taskId:${taskId}, cores:${JSON.stringify(cores)}`);
        ...
        try {
            ...
            logger.error(`coreTasks.service processCreateCoreTaskBatch: e: ${e.name}: ${e.message}`);
            failed = true;
            failedReason = e.message;
        }
    ...
    private async saveBatchStatus(taskId: string, cores: CoreItem[], failed: boolean, failedReason: string): Promise<void> {
        logger.debug(`coreTasks.service saveBatchStatus: in: taskId:${taskId}, failed:${failed}, failedReason:${failedReason}, cores:${JSON.stringify(cores)}`);
    ...
    public async list(count?: number, exclusiveStart?: CoreTaskListPaginationKey): Promise<[CoreTaskItem[], CoreTaskListPaginationKey]> {
        logger.debug(`coreTasks.service list: in: count:${count}, exclusiveStart:${JSON.stringify(exclusiveStart)}`);
