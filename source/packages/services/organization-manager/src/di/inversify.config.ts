...
container.bind<string>('aws.dynamodb.tables.accounts').toConstantValue(process.env.AWS_DYNAMODB_ORGANIZATION_TABLE);
container.bind<string>('aws.dynamodb.tables.gsi1').toConstantValue(process.env.AWS_DYNAMODB_TABLES_ORGANIZATION_GSI1);
container.bind<string>('aws.dynamodb.tables.gsi2').toConstantValue(process.env.AWS_DYNAMODB_TABLES_ORGANIZATION_GSI2);
container.bind<string>('aws.iam.role').toConstantValue(process.env.MANAGEMENT_ACCOUNT_ASSUME_ROLE);
container.bind<string>('aws.s3.controltower.bucket').toConstantValue(process.env.CONTROL_TOWER_MANIFEST_BUCKET);
container.bind<string>('aws.s3.controltower.prefix').toConstantValue(process.env.CONTROL_TOWER_MANIFEST_PREFIX);
container.bind<string>('aws.s3.controltower.filename').toConstantValue(process.env.CONTROL_TOWER_MANIFEST_FILENAME);
container.bind<boolean>('featureToggle.ous.create').toConstantValue(process.env.ENABLE_CREATE_OUS_FEATURE == 'true');
container.bind<boolean>('featureToggle.ous.delete').toConstantValue(process.env.ENABLE_DELETE_OUS_FEATURE == 'true');
container.bind<boolean>('featureToggle.accounts.create').toConstantValue(process.env.ENABLE_CREATE_ACCOUNTS_FEATURE == 'true');
container.bind<boolean>('featureToggle.accounts.delete').toConstantValue(process.env.ENABLE_DELETE_ACCOUNTS_FEATURE == 'true');
container.bind<string>('aws.organizations.suspendedOU').toConstantValue(process.env.SUSPENDED_OU);
container.bind<string>('aws.servicecatalog.product.owner').toConstantValue(process.env.SERVICE_CATALOG_PRODUCT_OWNER);
container.bind<string>('aws.servicecatalog.product.name').toConstantValue(process.env.SERVICE_CATALOG_PRODUCT_NAME);
...
container.bind<OrganizationalUnitsService>(TYPES.OrganizationalUnitsService).to(OrganizationalUnitsService);
container.bind<OrganizationalUnitsDao>(TYPES.OrganizationalUnitsDao).to(OrganizationalUnitsDao);
container.bind<OrganizationalUnitsAssembler>(TYPES.OrganizationalUnitsAssembler).to(OrganizationalUnitsAssembler);
...
container.bind<interfaces.Factory<AWS.ServiceCatalog>>(TYPES.ServiceCatalogFactory)
    .toFactory<AWS.ServiceCatalog>(() => {
        return () => {
            if (!container.isBound(TYPES.ServiceCatalog)) {
                const dc = new AWS.ServiceCatalog({ credentials: managementAccountCredentials, region: process.env.AWS_REGION });
...
container.bind<interfaces.Factory<AWS.Organizations>>(TYPES.OrganizationsFactory)
    .toFactory<AWS.Organizations>(() => {
        return () => {
            const awsOrganizationsRegion = 'us-east-1';
            if (!container.isBound(TYPES.Organizations)) {
                const organizations = new AWS.Organizations({ region: awsOrganizationsRegion, credentials: managementAccountCredentials });
                container.bind<AWS.Organizations>(TYPES.Organizations).toConstantValue(organizations);
            }
...
container.bind<interfaces.Factory<AWS.StepFunctions>>(TYPES.StepFunctions)
    .toFactory<AWS.StepFunctions>(() => {
        return () => {
            ...
            if (!container.isBound(TYPES.StepFunctions)) {
                ...
                container.bind<AWS.StepFunctions>(TYPES.StepFunctions).toConstantValue(StepFunctions);
            }
