...
export class ManifestDao {
    ...
    private static assembleOrganizationalUnitRegionItem(result: AWS.DynamoDB.DocumentClient.ItemList): AccountsByRegionListMap {
        logger.debug(`manifest.dao assembleOrganizationalUnitRegionItem: in: result: ${JSON.stringify(result)}`);
        const ouRegionItem = {};
        ...
        logger.debug(`manifest.dao assembleOrganizationalUnitRegionItem: exit: ${JSON.stringify(ouRegionItem)}`);
        return ouRegionItem;
    }


    public async getRegionAccountForOus(ids: string[]): Promise<RegionListByOrganizationalUnitMap> {
        const mapping = {}
        for (const id of ids) {
    ...
    public async getRegionAccountForOu(id: string): Promise<AccountsByRegionListMap> {
        ...
        logger.debug(`templates.dao getOrganizationalUnitRegionItemList: exit: ${JSON.stringify(itemList)}`);
        return itemList
    }
