...
export class ManifestService {
    ...
    public async updateManifestFile(): Promise<void> {
        ...
        const asYaml = yaml.dump(manifest, { noRefs: true, lineWidth: 300 }).replace('VERSION_PLACEHOLDER', '2021-03-15');
    ...
    private async assembleComponentsByOrganizationalUnitMap(organizationalUnits: string[]): Promise<ComponentsByOrganizationalUnitMap> {
        logger.debug(`manifest.service assembleComponentsByOrganizationalUnitMap: in: organizationalUnits:${JSON.stringify(organizationalUnits)}`);
        const map = {}
        ...
        logger.debug(`manifest.service assembleComponentsByOrganizationalUnitMap: exit: map:${JSON.stringify(map)}`)
        return map;
    }
    ...
    public async getComponentsManifest(): Promise<Manifest> {
        ...
        const componentsMap = await this.assembleComponentsByOrganizationalUnitMap(organizationalUnitItems.map(o => o.id))
        const accountsRegionsMap = await this.templatesDao.getRegionAccountForOus(organizationalUnitItems.map(o => o.id));
    ...
    private async uploadStreamToS3(body: NodeJS.ReadableStream): Promise<string> {
        const configurationKey = this.prefix === '' ? this.filename : `${this.prefix}/${this.filename}`
        logger.debug(`manifest.service uploadStreamToS3: in: bucket: ${this.bucket} key: ${configurationKey}`)
