...
export class AccountsDao {
    ...
    private createOrganizationalUnitsRegionMappingItems(organizationalUnit: string, regions: string[], accountId: string): AWS.DynamoDB.DocumentClient.TransactWriteItemList {
        return regions.map(region => {
            return {
                Put: {
                    TableName: this.accountsTable,
                    Item: {
                        "pk": createDelimitedAttribute(PkType.OrganizationalUnits, organizationalUnit),
    ...
    public async getAccountById(accountId: string): Promise<AccountsItem> {
        ...
        const getAccountResponse = await this._dc.query({
            TableName: this.accountsTable,
    ...
    private static assembleAccount(result: AWS.DynamoDB.DocumentClient.AttributeMap): AccountsItem {
        logger.debug(`accounts.dao assembleAccount: in: result: ${JSON.stringify(result)}`);
        const { pk, sk, status, regions, gsi2Key, email, ssoEmail, ssoFirstName, ssoLastName } = result
        let accountId
    ...
    private static assembleComponentsDeploymentStatus(result: AWS.DynamoDB.DocumentClient.ItemList): AccountComponentModel[] {
        logger.debug(`accounts.dao assembleComponentsDeploymentStatus: in: result: ${JSON.stringify(result)}`);
        ...
        logger.debug(`accounts.dao assembleComponentsDeploymentStatus: exit: ${JSON.stringify(componentDeploymentStatus)}`);
    ...
    public async listComponentsByAccount(accountId: string): Promise<AccountComponentModel[]> {
        ...
        const componentDeploymentStatusList = AccountsDao.assembleComponentsDeploymentStatus(queryResponse.Items);
        logger.debug(`accounts.dao listComponentsByAccount: exit componentDeploymentStatusList:${JSON.stringify(componentDeploymentStatusList)}`);
        return componentDeploymentStatusList
    }

    public async updateComponentByAccount(request: AccountComponentModel): Promise<void> {
        logger.debug(`accounts.dao updateComponentByAccount: in request:${JSON.stringify(request)}`);
    ...
    public async updateRegionsMappingForAccount(organizationalUnitId: string, accountId: string, regions: string[]): Promise<void> {
        logger.debug(`accounts.dao deleteRegionsMappingForAccount: in organizationalUnitId:${organizationalUnitId} accountId:${accountId}`);
        const queryResponse = await this._dc.query({
            TableName: this.accountsTable,
            KeyConditionExpression: 'pk = :pk and begins_with(sk,:sk)',
            ExpressionAttributeValues: {
                ':pk': createDelimitedAttributePrefix(PkType.OrganizationalUnits, organizationalUnitId),
                ':sk': createDelimitedAttributePrefix(PkType.Region, accountId),
            }
        }).promise();

        if (queryResponse.Items.length > 0) {
            logger.debug(`accounts.dao deleteRegionsMappingForAccount: queryResponse: ${JSON.stringify(queryResponse)}`)
            await this._dc.batchWrite({
                RequestItems: {
        ...
        await this._dc.transactWrite({
            TransactItems: [
                ...this.createOrganizationalUnitsRegionMappingItems(organizationalUnitId, regionSortedList, accountId)
            ],
        }).promise()
    ...
    public async deleteAccount(accountId: string): Promise<void> {
        ...
        const deleteRegionMappings = accountItem.regions === undefined ? [] : accountItem.regions?.map(region => {
            return {
                Delete: {
                    TableName: this.accountsTable,
                    Key: {
                        "pk": createDelimitedAttribute(PkType.OrganizationalUnits, organizationalUnit),
                        "sk": createDelimitedAttribute(PkType.Region, region, accountItem.name),
                    }
                }
            }
        })
            ...
            TransactItems: [{
                Delete: {
                    ...
                    Key: {
                        ...
                        sk: createDelimitedAttribute(PkType.OrganizationalUnits, organizationalUnit),
                    },
                }
    ...
    public async getAccountsInOu(ouName: string, count?: number, exclusiveStart?: AccountListPaginationKey): Promise<[AccountsItem[], AccountListPaginationKey]> {
        ...
        if (exclusiveStart?.organizationalUnitId && exclusiveStart?.accountName) {
            exclusiveStartKey = {
                sk: createDelimitedAttribute(PkType.OrganizationalUnits, exclusiveStart.organizationalUnitId),
        ...
        if (queryResponse.LastEvaluatedKey) {
            const organizationalUnitId = expandDelimitedAttribute(queryResponse.LastEvaluatedKey.pk)[1];
            const lastEvaluatedAccountName = expandDelimitedAttribute(queryResponse.LastEvaluatedKey.sk)[1];
            paginationKey = {
        ...
        logger.debug(`accounts.dao getAccountsInOu: exit: result: ${JSON.stringify(result)}, paginationKey: ${JSON.stringify(paginationKey)}`);
    ...
    public async createAccount(item: AccountsItem): Promise<AccountResource> {
        ...
        const { name: accountName, organizationalUnitId: organizationalUnit, accountId, ...rest } = item;

        await this._dc.put({
            TableName: this.accountsTable,
            Item: {
                pk: createDelimitedAttribute(PkType.Accounts, accountName),
                gsi2Key: accountId ? createDelimitedAttribute(PkType.Accounts, accountId) : undefined,
                sk: createDelimitedAttribute(PkType.OrganizationalUnits, item.organizationalUnitId),
                ...rest
            }
        }).promise()
        ...
        logger.debug(`accounts.dao create: exit: accountResource:${JSON.stringify(accountResource)}`);
