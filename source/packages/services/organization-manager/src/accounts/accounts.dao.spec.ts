...
describe('AccountsDao', function () {
    ...
    beforeEach(() => {
        ...
        instance = new AccountsDao("fakeAccountsTable", "fakeGsi1", "fakeGsi2", () => mockedDynamoDb)
    })
    ...
    it('createAccount: happy path', async () => {
        ...
        const mockPut = mockedDynamoDb.put = jest.fn().mockReturnValueOnce({ promise: () => Promise.resolve({}) })
    ...
    it('updateRegionsMappingForAccount: happy path', async () => {
        ...
        const mockedBatchWrite = mockedDynamoDb.batchWrite = jest.fn().mockReturnValueOnce({ promise: () => Promise.resolve({}) })

        mockedDynamoDb.transactWrite = jest.fn().mockReturnValueOnce({ promise: () => Promise.resolve({ Items:[] }) })

        await instance.updateRegionsMappingForAccount('fakeOu', 'fakeAccount', ['ap-southeast-2', 'us-west-1']);
